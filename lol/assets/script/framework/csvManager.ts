...
export class csvManager {
    ...
    csvTables: any = {}; //实际的数据，按json格式存储
    csvTableForArr: any = {}; //实际的数据，按数组格式存储
    tableCast: any = {}; //表格属性（第二行）
    tableComment: any = {};//表格说明（第一行）
    ...
    queryOne(tableName: string, key: string|number|null, value: any) {
        ...
        if (key) {
            for (const tbItem in table) {
                ...
                if (table[tbItem][key] === value) {
                    return util.clone(table[tbItem]);
                }
            }
        } else {
            return util.clone(table[value]);
        }
    }
