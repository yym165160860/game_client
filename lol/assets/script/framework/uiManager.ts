...
export class uiManager {
     ...
     */
    showDialog (panelPath: string, args?: any, cb?: Function, parentNode?:Node) {
        ...
        if (this.dictSharedPanel.hasOwnProperty(panelPath)) {
            ...
            if (isValid(panel)) {
                panel.parent = parentNode || find("Canvas");
                panel.active = true;
                let script = panel.getComponent(scriptName) as IPanel;
                if (script.show) {
                    script.show.call(script, args);
                }
        ...
        resourceUtil.createUI(panelPath, parentNode, (err, node) => {
            //判断是否有可能在显示前已经被关掉了？
    ...
    showTips (content: string, cb?: Function) {
        ...
        if (now - this.showTipsTime < SHOW_STR_INTERVAL_TIME) {
            ...
            const self = this;
            setTimeout(function (tipsLabel:any, callback?:Function) {
    ...
    _showTipsAni(content: string, cb?: Function) {
        ...
        resourceUtil.getUIPrefabRes('common/fadeframe', (err, prefab) => {
            if (err) {
                return;
            }
            ...
            tipScript && tipScript.show(content, cb);
        });
    }

    showPopup(content: string, btnText?: Array<string|null>|null, nSeconds?: number, cb?: Function) {
        resourceUtil.getUIPrefabRes('common/popup', (err, prefab) => {
            if (err) {
                return;
            }

            let popupNode = poolManager.instance.getNode(prefab!, find("Canvas")!) as Node;
            let script = popupNode.getComponent(popup)!;
            script.show(content, btnText, nSeconds, cb);
        });
    }

    showPopupTips(content: string, cb?: Function) {
        this.showPopup(content, null, 0, cb);
    }
}
